...
# We have added a column named row number and using that in join as opposed to date, because trading is off on weekends
...
# We will use self join for the same.
...
# Keeping track of our tables

select *
from bajaj2
limit 5;

# Now we update the signal column based on the values of flag and flagp.
...
# We need to look for a change in the sign of flag. (positive to negative and vice versa) 
# Need to bring the previuos flag in the same row for the purpose of comparison. We will use self join for the same.
